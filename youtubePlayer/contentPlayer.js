...
if (urlValid()) {
    ...
    chrome.runtime.onMessage.addListener(async (msg) => {
        console.log("action player = " + msg.action)
        if (msg.action === "DataEpisodeResponse") {
            if (msg.url === localStorage)
            let content = JSON.parse(msg.content)
            let count = 0
            while (count < 5) {
                let video = document.querySelector("video")
                if (video.currentTime <= content.time) {
        ...
        try {
            if (!changePLayer || document.getElementsByClassName("controls").length == 0) {
                ...
                chrome.runtime.sendMessage({
                    from: "iframe",
                    action: "init",
                    message: globalThis.location.origin + globalThis.location.pathname
                });
